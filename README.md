...
`debrief-result` explains one complete or partial result, connecting technical changes to business meaning and resolving material ambiguity with the user; `project-advisor` evaluates broader direction and priorities. Accepting advice does not automatically create tickets or change Notion/Linear. An explicit execution request carries the chosen decision into the relevant execution workflow.
...
Both implementation skills finish with an explicit verdict: a verified complete ticket is ready to prepare a PR and proceed toward merge, with actual PR, merge, and tracker progress stated separately. An incomplete handoff explains the original business problem, completed business and technical progress, the remaining outcome, its obstacle and proposed resolution, and whether to continue the ticket, use or propose separate work, or pause until a specific condition is met. A reporting verdict does not itself publish, merge, create a ticket, or change tracker state.
...
/debrief-result                    # Explain a complete or partial result in business and technical context
