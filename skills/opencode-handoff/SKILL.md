...
description: "Use when any coding agent needs to hand the current repository task to OpenCode, especially before a usage or context limit interrupts ongoing work."
user-invocable: true
allowed-tools: Bash
---
...
Export the current task state into a local handoff bundle that OpenCode can take over with `/takeover` (or `$takeover`). Works from Claude Code, Codex, Antigravity, Cursor, Grok Build, or any other coding agent.
...
2. Locate this skill's bundled `scripts/opencode-handoff.sh` and run it from the current project with a real source name and the summary path. Examples:

   Claude Code plugin install:

   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/skills/opencode-handoff/scripts/opencode-handoff.sh" \
     --source claude \
   ...
   ```bash
   bash /path/to/cktk/skills/opencode-handoff/scripts/opencode-handoff.sh \
     --source codex \
     --summary /tmp/agent-summary.md \
     --no-session
   ```
   ...
   Useful flags:
   - `--source AGENT` — `claude`, `codex`, `antigravity`, `cursor`, `grok`, etc.
