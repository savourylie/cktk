...
usage() {
  cat <<'EOF'
Usage: opencode-handoff [--tail-lines N] [--target AGENT] [--source AGENT] [--out PATH]
                    [--summary PATH] [--no-session] [--require-session]

Export the current git state (and optional Claude Code session / agent summary)
into a local handoff bundle for OpenCode takeover.
EOF
}

die() {
  printf 'opencode-handoff: %s\n' "$*" >&2
  exit 1
}
...
snapshot_dir="$(mktemp -d "${TMPDIR:-/tmp}/opencode-handoff-snapshot.XXXXXX")"
work_dir=""
latest_tmp=""
...
print("OpenCode" if name == "Opencode" else name)
PY
)"
source_display="$(
...
if [[ -n "$out_rel" ]]; then
  if ! git -C "$repo_root" check-ignore -q -- "$out_rel/"; then
    printf 'opencode-handoff: WARNING: %s is not ignored by git. Add %s/ to .gitignore because handoff bundles can contain sensitive transcripts and diffs.\n' \
      "$out_rel" "$out_rel" >&2
  fi
fi
