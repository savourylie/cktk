...
Explain what the work achieved, why it matters to the project, and what is still unresolved. A result may be complete, partial, blocked, or limited to findings. Start from the current conversation; no ticket number is required.
...
Stay focused on the selected result. Whole-project strategy and backlog prioritization belong to `project-advisor`.
...
Recover facts before asking the user to repeat them. Ask only when uncertainty about the result, goal, scope, terminology, or business rule materially changes the explanation or recommendation. State what is known, offer the plausible interpretations and their consequences when useful, and ask the focused missing question. Reuse prior answers; silence does not resolve a required question. Do not demand a generic “do you understand?” confirmation when the context is clear. If the user corrects the framing, revise the explanation from that shared context.

## Safety rules

- Read-only: never edit files, tickets, Linear, Notion, or git; never commit or run mutating commands.
- Do not invoke implementation, ticket creation/update, review, quiz, commit, PR, or worktree skills. Explain or recommend next steps only.
